...
def inicializaShaders():
    ...
    fragment_shader = """
        ...
        bool esta_na_sombra(vec3 luz, vec3 ponto_de_impacto, vec3 normal) {
            vec3  dir_luz = normalize(luz - ponto_de_impacto);
            float dist_ate_luz = length(luz - ponto_de_impacto);
        ...
        void main() {
            ...
            for (int bounce = 0; bounce < 4; bounce++) {
                ...
                vec3 cor_local = vec3(0.0);

                vec3 ambiente = cor_mat * 0.15;
                cor_local += ambiente;

                for (int i = 0; i < 5; i++) {
                    if (!esta_na_sombra(luzes[i], ponto, normal)) {
                        vec3 L = normalize(luzes[i] - ponto);
                        vec3 V = normalize(cam_origem - ponto);

                        float Kd = max(dot(normal, L), 0.0);
                        vec3 difusa = cor_mat * Kd * 0.75;

                        vec3 R = reflect(-L, normal);
                        float Ks = pow(max(dot(R, V), 0.0), 32.0);
                        vec3 especular = vec3(1.0) * Ks * 0.6;

                        cor_local += difusa + especular;
                    }
                }

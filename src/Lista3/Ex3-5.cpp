...
const GLchar *fragmentShaderSource = R"(
    ...
    void main() {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            vec3 diffuse = diff * lights[i].color * inputColor.rgb;
...
struct Light {
    ...
    Light(glm::vec3 lightPos, glm::vec3 lightColor) : position(lightPos), color(lightColor) {}
};
...
struct Material {
    ...
    Material(glm::vec3 matColor, float matKa, float matKd, float matKs, float matShininess) : color(matColor), Ka(matKa), Kd(matKd), Ks(matKs), shininess(matShininess) {}
};
...
int main() {
	...
	while (!glfwWindowShouldClose(window)) {
        ...
        for (int i = 0; i < 5; i++) {
            string posName = "lights[" + to_string(i) + "].position";
            string colorName = "lights[" + to_string(i) + "].color";

            glUniform3fv(glGetUniformLocation(shaderID, posName.c_str()), 1, glm::value_ptr(lights[i].position));
            glUniform3fv(glGetUniformLocation(shaderID, colorName.c_str()), 1, glm::value_ptr(lights[i].color));
        }

        glUniform3f(glGetUniformLocation(shaderID, "ambientLightColor"), ambientLight.color.x, ambientLight.color.y, ambientLight.color.z);

        glUniform1f(glGetUniformLocation(shaderID, "ks"), material.Ks);
